...
MPJCONFIGFILE=mpj.conf
#MACHINESFILE="mpjmachines.$LSB_JOBID"
##total_ncpus=$LSB_DJOB_NUMPROC
#total_ncpus=`bash creatempjmachinefile.sh $MACHINESFILE`

#JVMARGS="-Xmx${JVMMAXMEM}m"

#echo "runmpj.sh"
#mpjrun.sh -np 4 -cp $CLASSPATH ch.ethz.khammash.hybridstochasticsimulation.MainMPJ $1

mainclass=ch.ethz.khammash.hybridstochasticsimulation.MainMPJ
#runmpj.sh $MPJCONFIGFILE  ch.ethz.khammash.hybridstochasticsimulation.MainMPJ $1
java -cp .:$MPJ_HOME/lib/mpj.jar:$CLASSPATH $mainclass 0 mpj.conf niodev test.xml &
java -cp .:$MPJ_HOME/lib/mpj.jar:$CLASSPATH $mainclass 1 mpj.conf niodev test.xml
#java -cp .:$MPJ_HOME/lib/mpj.jar $mainclass 0 $MPJCONFIGFILE niodev &
#java $mainclass 1 $MPJCONFIGFILE niodev
#java $mainclass 1 $MPJCONFIGFILE niodev $1

#./runmpj.sh $MPJCONFIGFILE "$JVMARGS" ch.ethz.khammash.hybridstochasticsimulation.MainMPJ $2
