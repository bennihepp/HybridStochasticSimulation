...
public class CPSpeciesBalance {
	...
	public int[] search_solution(int[] min_alpha, int[] max_alpha, int[] min_beta, int[] max_beta, double gamma) {
		// Define FD store
		...
		for (int i = 0; i < min_alpha.length; i++)
			alpha[i] = new IntVar(store, "alpha" + i, min_alpha[i], max_alpha[i]);
		IntVar[] beta = new IntVar[r0];
		for (int i = 0; i < min_beta.length; i++)
			beta[i] = new IntVar(store, "beta" + i, min_beta[i], max_beta[i]);
		IntVar[] rho = build_rho(store, alpha, beta, gamma);

		Vector<PrimitiveConstraint[]> constraints = build_species_balance_constraints(store, alpha, beta, rho, gamma);
		...
		SelectChoicePoint<IntVar> select = new InputOrderSelect<IntVar>(store, v_array, new IndomainMiddle<IntVar>());
		...
		constraints = build_species_balance_constraints(store, alpha, beta, rho, gamma);
		...
		select = new InputOrderSelect<IntVar>(store, v_array, new IndomainMiddle<IntVar>());
		...
		for (int r = 0; r < r0; r++) {
			...
			for (int s = 0; s < s0; s++) {
				if (this.stoch_matrix[r][s] < 0) {
					...
					store.impose(new XmulCeqZ(alpha[s], -this.stoch_matrix[r][s], p));
					vlist.add(p);
				}
			}
	...
	Vector<PrimitiveConstraint[]> build_species_balance_constraints(Store store, IntVar[] alpha, IntVar[] beta, IntVar[] rho,
			double gamma) {
		int s0 = alpha.length;
		int r0 = beta.length;
