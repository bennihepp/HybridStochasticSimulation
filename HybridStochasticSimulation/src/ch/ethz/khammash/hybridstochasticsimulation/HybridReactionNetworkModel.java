...
public class HybridReactionNetworkModel implements FirstOrderDifferentialEquations, ReactionNetworkModel {
	...
	private void init(HybridReactionNetwork hrn) {
		...
		// deterministicReactionSpeciesIndices = new
		// int[numOfDeterministicReactions];
		// deterministicReactionChoiceIndices = new
		// ArrayList<int[]>(numOfDeterministicReactions);
		// deterministicReactionParameters = new
		// double[numOfDeterministicReactions];
		...
		for (int r = 0; r < hrn.numOfReactions; r++) {
			...
			for (int s = 0; s < hrn.numOfSpecies; s++) {
				if (ct[r][s] == HybridReactionNetwork.ConvergenceType.DETERMINISTIC) {
					...
					// For stochastic reactions changing species s, alpha(s) ==
					// 0 so we
					...
					// = Math.pow(hrn.getN(), -hrn.getAlpha(s)) *
					// hrn.getStochiometry(s, r);
				}
			}
