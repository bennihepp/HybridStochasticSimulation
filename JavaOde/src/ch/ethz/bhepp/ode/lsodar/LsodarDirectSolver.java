...
//    	checkArgument(absTol >= 0);
    ...
    public void initialize(Ode ode, EventFunction ef, StateObserver stateObserver, EventObserver eventObserver) throws InitializationException {
    	if (initialized)
    		dispose();
...
//    	g = gBuffer.asDoubleBuffer().array();
    	...
    	try {
    		...
    		throw new InitializationException("Failed to initialize native LsodarDirectSolver", e);
    	}
        if (jni_pointer == 0)
        	throw new InitializationException("Failed to initialize native LsodarDirectSolver");
        initialized = true;
    }
...
//    		g = null;
	...
	public double integrate(double t0, double[] x0, double t1) {
		throw new UnsupportedOperationException("Not implemented in LsodarDirectSolver");
	}
    		...
    		while (true) {
        		...
        		if (t < tNext)
            		if (eventOccured()) {
            			...
            			throw new IntegrationException("Integration of ODE failed");
        		if (t == tNext)
        			stateObserver.report(t, x);
